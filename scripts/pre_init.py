...
if not defines.DISABLE_INFLUXDB and not os.path.exists(defines.INFLUXDB_DIR):
    ...
    os.system('docker run -d --name=influx_tmp ' + defines.INFLUXDB_IMAGE)
    os.system('docker cp -a influx_tmp:/etc/influxdb/ ' + defines.INFLUXDB_DIR + '/etc')
    os.system('docker rm -f influx_tmp')
...
if not defines.DISABLE_GRAFANA and not os.path.exists(defines.GRAFANA_DIR):
    ...
    with open(os.path.join(defines.GRAFANA_DIR, 'new'), 'w') as file_writer:
        file_writer.write(time.asctime( time.localtime(time.time())))
    os.system('docker run -d --user root --name=grafana_tmp ' + defines.GRAFANA_IMAGE)
    os.system('docker cp -a grafana_tmp:/etc/grafana/ ' + defines.GRAFANA_DIR + '/etc')
    os.system('docker cp -a grafana_tmp:/var/lib/grafana/ ' + defines.GRAFANA_DIR + '/data')
    os.system('docker cp -a grafana_tmp:/var/log/grafana/ ' + defines.GRAFANA_DIR + '/log')
    os.system('docker rm -f grafana_tmp')
...
if not defines.DISABLE_MYSQL and not os.path.exists(defines.MYSQL_DIR):
    ...
    with open(os.path.join(defines.MYSQL_DIR, 'new'), 'w') as file_writer:
        file_writer.write(time.asctime( time.localtime(time.time())))
    os.system('docker run -d --name=mysql_tmp ' + defines.MYSQL_IMAGE)
    os.system('docker cp -a mysql_tmp:/var/lib/mysql/ ' + defines.MYSQL_DIR + '/data')
    os.system('docker cp -a mysql_tmp:/etc/mysql/conf.d/ ' + defines.MYSQL_DIR + '/config')
    os.system('docker rm -f mysql_tmp')
...
if not defines.DISABLE_NGINX_PHP and not os.path.exists(defines.NGINX_DIR):
    ...
    os.system('docker run -d --name=nginx_tmp ' + defines.NGINX_IMAGE)
    os.system('docker cp -a nginx_tmp:/usr/share/nginx/html/ ' + defines.NGINX_DIR + '/html')
    os.system('docker cp -a nginx_tmp:/var/log/nginx/ ' + defines.NGINX_DIR + '/logs')
    os.system('docker rm -f nginx_tmp')
